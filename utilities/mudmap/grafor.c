...
square_x, square_y, room_color[type]);
					...
					if (room[i].d[j])
						...
						if (k==-1)
								...
								if (!((newx==room[k].x)&&(newy==room[k].y)&&(newz==room[k].z)))
									{
									//printf("E %i->%i ",room[i].vnum,room[k].vnum);
									//fflush(NULL);
									room[i].error=1;
									}
								}
...
for (y=y0+dy; y>y0; y--)
	...
	for (x=x0; x<(x0+dx); x++)
		...
		switch(c)
			{
			case '*': square(1, room[index[megamap[x1][y1][z1]]].d); break;
			case '#': square(2, room[index[megamap[x1][y1][z1]]].d); break;
			case 'E': square(3, room[index[megamap[x1][y1][z1]]].d); break;
			default: if ((x1>=0)&&(y1>=0)&&(z1>=0)) square(0, room[index[megamap[x1][y1][z1]]].d);
				 else square(0, zerodir);
			}
		}
