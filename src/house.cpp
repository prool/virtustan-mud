...
void Clan::ClanLoad()
{
	...
	for (std::list < std::string >::const_iterator it = clanIndex.begin(); it != clanIndex.end(); ++it)
	{
		...
		if (0/*tempClan->exp_history.need_destroy()*/) // prool: временно неактивные кланы не удаляем
		{
...
void Clan::ChestUpdate()
{
	...
	for (ClanListType::const_iterator clan = Clan::ClanList.begin(); clan != Clan::ClanList.end(); ++clan)
	{
		...
		if (0/*(*clan)->bank < 0*/) // prool: коммунизм
		{
			(*clan)->bank = 0;
...
int Clan::ChestTax()
{
return 0; // prool: коммунизм, free chest tax
	OBJ_DATA *temp, *chest;
	int cost = 0;
	int count = 0;
...
{
return 0; // prool: коммунизм, free ingr chest tax
	if (!ingr_chest_active())
	{
		return 0;
	}
