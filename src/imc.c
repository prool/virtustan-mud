...
/*
   ...
   -- Xorith
*/
char                                   *imcstrrep(const char *src, const char *sch,
						  const char *rep)
{
...
const char                             *color_itom(const char *txt, CHAR_DATA *ch)
{
	...
	return "";
#if 1 // prool
    if (0/*IMCIS_SET(IMCFLAG(ch), IMC_COLORFLAG)*/) {
	imcstrlcpy(tbuf, txt, LGST);
	for (color = first_imc_color; color; color = color->next)
...
void imc_to_pager(const char *txt, CHAR_DATA *ch)
{
    ...
    snprintf(buf, LGST * 2, "%s", color_itom(txt, ch));
#if defined(IMCSMAUG) || defined(IMCCHRONICLES)
...
char                                   *imc_mudof(const char *src)
{
    ...
    if (!(person = (char *)strchr(src, '@'))) // prool
...
void imc_new_reminfo(const char *mud, const char *version, const char *netname, const char *url,
		     const char *path)
{
    REMOTEINFO                             *p,
                                           *mud_prev;
...
void imc_new_channel(const char *chan, const char *owner, const char *ops, const char *invite,
		     const char *exclude, bool copen, int perm, const char *lname)
{
    IMC_CHANNEL                            *c;
...
int imcfread_number(FILE * fp)
{
	...
	imcnumber = imcnumber * 10 + c - '0';
	c = getc(fp);
    }

    if (sign)
	imcnumber = 0 - imcnumber;

    if (c == '|')
	imcnumber += imcfread_number(fp);
    else if (c != ' ')
	ungetc(c, fp);
...
char                                   *imc_getData(char *output, const char *key,
						    const char *packet)
{
    int                                     current = 0;
    size_t                                  i = 0;
...
void imc_write_buffer(const char *txt)
{
     ...
     */
    while (this_imcmud->outtop + outlen >= this_imcmud->outsize) {
     ...
     * Copy.
     */
    strncpy(this_imcmud->outbuf + this_imcmud->outtop, output, outlen);	/* Leave this one alone! BAD THINGS(TM) will
									 * happen if you don't! */
    this_imcmud->outtop += outlen;
...
void imc_write_packet(IMC_PACKET *p)
{
     ...
     */
    snprintf(txt, IMC_BUFF_SIZE, "%s %lu %s %s %s", p->from, ++imc_sequencenumber,
	     this_imcmud->localname, p->type, p->to);
...
PFUN(imc_recv_tell)
{
    ...
    if (!(vic = imc_find_user(imc_nameof(q->to))) /*|| IMCPERM(vic) < IMCPERM_MORT*/) { // prool
...
PFUN(imc_recv_emote)
{
	...
	if (d->connected == CON_PLAYING
	    ...
	    /*&& IMCPERM(ch) >= level*/) { // prool
	    imc_printf(ch, "~p[~GIMC~p]~! %s ~c%s\r\n", imc_speaker_name(imcgetname(q->from)),
		       txt);
	}
    }
}
...
void update_imchistory(IMC_CHANNEL *channel, char *message)
{
	...
	if (channel->history[x] == NULL) {
	    ...
	    //log_imc("%s", imc_strip_colors(channel->history[x])); // prool
...
char                                   *process_plrline(char *plrrank, char *plrflags,
							char *plrname, char *plrtitle)
{
...
char                                   *process_immline(char *plrrank, char *plrflags,
							char *plrname, char *plrtitle)
{
    static char                             pline[LGST];
...
char                                   *process_who_template(char *head, char *tail,
							     char *plrlines, char *immlines,
...
char                                   *imc_assemble_who(void)
{
    ...
    imcstrlcpy(master,
	       process_who_template(head, tail, plrlines, immlines, plrheader, immheader),
	       LGST);
...
void imc_process_finger(const char *from, const char *type)
{
	...
	snprintf(buf + strlen(buf), IMC_BUFF_SIZE - strlen(buf),
		 ...
		 "None", (IMC_HOMEPAGE(victim)
			  && IMC_HOMEPAGE(victim)[0] != '\0') ? IMC_HOMEPAGE(victim) : "None",
		 IMC_ICQ(victim), (IMC_AIM(victim)
				   && IMC_AIM(victim)[0] != '\0') ? IMC_AIM(victim) : "None",
		 (IMC_YAHOO(victim)
		  && IMC_YAHOO(victim)[0] != '\0') ? IMC_YAHOO(victim) : "None",
...
void imc_parse_packet(const char *packet)
{
	...
	imc_new_reminfo(imc_mudof(p->from), "Unknown", this_imcmud->network, "Unknown",
			p->route);
...
void imc_process_authentication(const char *packet)
{
	...
	snprintf(response, LGST, "SHA256-AUTH-RESP %s %s version=%d", this_imcmud->localname,
		 cryptpwd, IMC_VERSION);
...
bool imc_read_buffer(void)
{
    ...
    for (i = 0; this_imcmud->inbuf[i] != '\0'
	 && this_imcmud->inbuf[i] != '\n' && this_imcmud->inbuf[i] != '\r' && i < IMC_BUFF_SIZE;
	 i++) {
...
bool imc_read_socket(void)
{
	...
	if (++loop_count > 100)				       // Yay hackish error traps! 100 loops likely means she's 
							       // 
	    // stuck. Break out with whatever is there.
	    break;
	...
	if (nRead > 0) {
	    ...
	    imclog("%s: Descriptor error on #%d: %s", __FUNCTION__, this_imcmud->desc,
		   strerror(iErr));
	    return FALSE;
	}
    }
...
void imc_loop(void)
{
	...
	while (imc_read_buffer()) {
		...
		default:
		case IMC_OFFLINE:
		case IMC_AUTH1:			       /* Auth1 can only be set when still trying to contact *
							        * the server */
		    break;

		case IMC_AUTH2:			       /* Now you've contacted the server and need to process * 
...
void imcfread_config_file(FILE * fin)
{
	...
	switch (word[0]) {
		...
		if (!strcasecmp(word, "End")) {
		     ...
		     */
		    snprintf(lbuf2, LGST, "%s (%s)", this_imcmud->base, IMC_VERSION_STRING);
...
bool imc_read_config(int desc)
{
	 ...
	 */
	snprintf(cbase, SMST, "%s (%s)", this_imcmud->base, IMC_VERSION_STRING);
...
bool imc_server_connect(void)
{
	...
	imcwait = 100;					       /* So it will try again according to the reconnect
							        * count. */
	return FALSE;
    }
...
IMC_CMD(imcchanlist)
{
	...
	imc_printf(ch, "~c%-15.15s ~%c%-*.*s ~B%-15.15s ~b%-7s %s\r\n", c->name, col,
		   ...
		   imcperm_names[c->level],
		   c->refreshed ? (c->open ? "~gOpen" : "~yPrivate") : "~Runknown");
	count++;					       /* Keep a count -- Xorith */
    }
    /*
     * Show the count and a bit of text explaining the red color -- Xorith 
     */
    imc_printf(ch, "~W%d ~cchannels found.\r\n", count);
    imc_to_char("~RRed ~clocal name indicates a channel not being listened to.\r\n", ch);
}
...
IMC_CMD(imclist)
{
    ...
    imcpager_printf(ch, "~c%-20.20s ~Y%-15.15s %-5.5s ~B%-20.20s~! ~g%-15.15s ~G%s~!\r\n",
		    "Name", "Address", "Port", "Version", "Network", "Server");

    for (p = first_rinfo; p; p = p->next, count++) {
	/*
	 * Skip muds that don't match the pattern we gave (if any)
	 */
	if (argument && *argument) {
	    if (imcstr_prefix(argument, p->name))
		continue;
	}
	*serverpath = *netname = '\0';
	...
	imcpager_printf(ch, "~%c%-20.20s ~Y%-15.15s %-5.5s ~B%-20.20s ~g%-15.15s ~G%s~!\r\n",
			p->expired ? 'R' : 'c', p->name, p->host, p->port, p->version, netname,
			serverpath);
    }

    imcpager_printf(ch, "\r\n~WRed mud names indicate connections that are down.\r\n");
    imcpager_printf(ch, "~W%d muds on %s found.\r\n", count, this_imcmud->network);
}
...
char                                   *imc_act_string(const char *format, CHAR_DATA *ch,
						       CHAR_DATA *vic)
{
...
bool imc_command_hook(CHAR_DATA *ch, const char *command, const char *argument)
{
	...
	imcbug("Permission %d vs. %d", IMCPERM(ch), IMCPERM_NONE);
	return FALSE;
    }
#if defined(IMCCIRCLE)
    /*
